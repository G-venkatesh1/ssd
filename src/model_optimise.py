...
from src.utils_optimise import generate_dboxes, Encoder
...
class Base(nn.Module):
    ...
    def bbox_view(self, src, loc, conf):
        ...
        ploc, plabel = locs.float(), confs.float()
        # ploc, plabel = locs[:,:,:1000], plabel[:,:,:1000]
        
        #post processing
        
        # dboxes = generate_dboxes_high_res()
        ...
        if int(ploc.shape[0])>1:
            for idx in range(ploc.shape[0]):
                ...
                loc_i, label_i, prob_i = [r.cpu() for r in result]
                loc_list.append(loc_i)
                label_list.append(label_i)
                prob_list.append(prob_i)
              
            return loc_list, label_list, prob_list
...
class SSD(Base):
    ...
    def forward(self, x):
        ...
        locs, confs, probs = self.bbox_view(detection_feed, self.loc, self.conf)
        return locs, confs, probs
