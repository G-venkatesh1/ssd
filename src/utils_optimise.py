...
class Encoder(object):
    ...
    def decode_single(self, bboxes_in, scores_in, nms_threshold, max_output, max_num=200):
        ...
        for i, score in enumerate(scores_in.split(1, 1)):
            ...
            mask_sorted, indices_sorted= score.sort(descending=True)
            indices_new= torch.where(mask_sorted < 0.05)
            
            if len(indices_new) == 0: continue
            cut_idx = int(indices_new[0][0])

            score= mask_sorted[0:cut_idx]
            indices_sorted = indices_sorted[0:cut_idx]
            bboxes= bboxes_in[indices_sorted, :]  
        ...
        # print(score_aver/80)
        # return bboxes_out[max_ids, :], labels_out[max_ids], scores_out[max_ids], score_aver/80
